...
4. Separate **$150 Verification** Checkout/PaymentIntent (product: SBS Verification). SBS absorbs Stripe on that charge. Success creates an inbound FedEx label job seller → **South Bay Saddlery, Attn Jeff Sarault, 32 Glory Road, Santa Rosa Beach FL 32459** (Jeff-locked default; `VERIFY_SHIP_TO_*` overrides). Not Inlet Beach.
...
| `VERIFY_SHIP_TO_NAME` / `ATTN` / `STREET` / `CITY` / `STATE` / `ZIP` / `HOURS` | Inbound verify ship-to. Defaults: South Bay Saddlery / Attn Jeff Sarault / 32 Glory Road / Santa Rosa Beach FL 32459 / Mon–Fri 9am–5pm CT. Env overrides. Not Inlet Beach. Fail-verify returns ship to the seller, not this address. |
